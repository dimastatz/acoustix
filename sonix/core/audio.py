...
def vad_with_pyanote(
    path: str, threshold: float = 0.5, min_silence_len_sec: float = 0.25
) -> List[Dict]:
    """Voice activity detection using `pyannote.audio` when available.

    - If `pyannote.audio` is installed and a compatible model can be
      instantiated, use it to get speech probabilities and threshold
      them to produce speech segments.
    - If any error occurs (ImportError or runtime), fall back to a
      simple RMS-based VAD using `librosa`.

    Returns a list of segment dicts with `start_time_sec`,
    `end_time_sec` and `label` keys (same structure used elsewhere).
    """
    # Delegate to helpers so this wrapper stays small (avoids too-many-locals).
    try:
        return _vad_via_pyanote(path, threshold, min_silence_len_sec)  # type: ignore
    except (ImportError, RuntimeError, ModuleNotFoundError, AttributeError, OSError):
        return _vad_rms(path, min_silence_len_sec)


def _vad_via_pyanote(
    path: str, threshold: float, min_silence_len_sec: float
) -> List[Dict]:
    """Attempt VAD using `pyannote.audio.Inference`.

    Import is local to avoid failing module import when pyannote isn't
    installed. The import line intentionally lives here.
    """
    # pylint: disable=import-outside-toplevel,import-error
    from pyannote.audio import Inference  # type: ignore

    inf = Inference("pyannote/voice-activity-detection", device="cpu")
    result = inf({"uri": "audio", "audio": path})

    probs = getattr(result, "data", None)
    window = getattr(result, "sliding_window", None)
    if probs is None or window is None:
        raise RuntimeError("Unexpected pyannote output")

    data, sr = librosa.load(path, sr=None, mono=True)

    entries = _probs_to_entries(probs, window, sr, threshold)

    min_gap = int(min_silence_len_sec * sr)
    collapsed = merge_intervals(entries, min_gap)
    return entries_to_segments(collapsed, sr, int(data.shape[0]), pad_sec=0.0)


def _vad_rms(path: str, min_silence_len_sec: float) -> List[Dict]:
    """Simple RMS-based VAD fallback implemented with librosa."""
    data, sr = librosa.load(path, sr=None, mono=True)
    ...
    thresh = max(np.median(rms) * 0.5, np.mean(rms) * 0.3)
    mask = rms >= thresh

    entries = _rms_mask_to_entries(mask, times, int(data.shape[0]), frame_length, sr)

    min_gap = int(min_silence_len_sec * sr)
    collapsed = merge_intervals(entries, min_gap)
    return entries_to_segments(collapsed, sr, int(data.shape[0]), pad_sec=0.01)


def _probs_to_entries(probs, window, sr: int, threshold: float) -> Intervals:
    """Convert pyannote probs + sliding window into sample-indexed entries."""
    entries: Intervals = []
    for i, p in enumerate(probs.flatten()):
        start = float(window[i].start)
        end = float(window[i].end)
        is_speech = float(p) >= float(threshold)
        entries.append((int(start * sr), int(end * sr), bool(is_speech)))
    return entries


def _rms_mask_to_entries(
    mask, times, data_len: int, frame_length: int, sr: int
) -> Intervals:
    """Convert RMS boolean mask into sample-indexed entries."""
    entries: Intervals = []
    for i, val in enumerate(mask):
        s = int(times[i] * sr)
        e = int(min(data_len, (times[i] + (frame_length / sr)) * sr))
        entries.append((s, e, bool(val)))
    return entries
